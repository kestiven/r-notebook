...
Cargue cada set de datos y nombre lo según cada archivo.
...
Utilice *left_joins* en esté ejercicio para asegurarse de mantener todas las preguntas, incluso aquellas sin un *tag* correspondiente.
...
### 1.3 Utilice *replace_na* para cambiar los NA en la columna *tab_name* a "only-r".
...
Realice un breve análisis, para ello utilice verbos de la familia *dplr* como *group by, summarise, arrange* y averigüe el score promedio de las preguntas más frecuentes.
...
### 2.2 Aplique *summarise* para obtener el score promedio de cada pregunta y asígnele el nombre *mean_score*.
...
### 3.1 Utilice *inner_join* para combinar las tablas *questions* y *answers*, luego aplique los sufijos "\_*question" y "*\_answer", respectivamente.
...
```{r}
questions$creation_date = strptime(questions$creation_date, format = "%Y-%m-%d")
answers$creation_date = strptime(answers$creation_date, format = "%Y-%m-%d")
questions %>%
  inner_join(answers, by = c("id" = "question_id"), suffix = c("_question", "_answer")) %>%
...
### 5.1 Combine *questions_tags* con *question_answer_counts* usando *inner_join*.
